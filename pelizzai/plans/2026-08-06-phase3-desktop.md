...
9. The plugin bar renders every catalog plugin from a plugin id list in `AppState` — ratified in the Task 10 execution interview on 2026-08-08 — rejected hiding the catalog and showing only the active plugin; main derives the ids from `CATALOG`, the renderer never imports core.
10. Task 10 completes the real settings/config/keys backend — ratified in the Task 10 execution interview on 2026-08-08 after the discovery that the Task 5 settings/config/keys handlers were left as legacy `{ok,data,status}` stubs with no key vault and no provider wiring. Main returns the contract's camelCase `Result<AppState>`, persists guitar/model/locale/theme, and adds a `safeStorage`-vault-backed `keys:save`/`keys:remove` returning `KeyInfo`; this supersedes the plan's renderer-only reading of Task 10.
11. Task 10 also wires real AI provider clients — ratified in the Task 10 execution interview on 2026-08-08 — the desktop adds `openai` + `@anthropic-ai/sdk` (same versions the CLI pins) and `getProviders()` builds real clients from vault-decrypted keys (mirroring the CLI's `wireProviders`), so chat works from a saved key; the core `key-store` is configured against the same SQLite file with a `safeStorage`-backed `Vault`.
