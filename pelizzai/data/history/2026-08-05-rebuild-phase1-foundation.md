# rebuild-phase1-foundation — 2026-08-05
...
- effect: write-local
- risk: medium
- overlays: pelizzai-oswap (applied 2026-08-05; frontend/documenting-features recorded at
  kickoff before Phase 1 scoped to core/platform-node/cli only — zero UI or new-docs surface in
  the closed diff, confirmed at closeout, correctly not applicable here)
- audience: technical
...
- Final review ✅ 2026-08-05 — independent full-range review (b517c7c..d8042a9): no Critical,
  2 Important (i18n pt.json gap, AppInfo Windows-only candidates) fixed at 945c95f and
  independently re-verified PASS; 1 Minor accepted (documented `any` workaround)
- SEALED at 945c95f — 125 tests passing (90 core + 24 platform-node + 11 cli), 0 failures, clean
  typecheck across all 4 packages, working tree clean, kickoff ratified.

## Known gaps carried forward (see plan's "Exposed material gaps" for full detail)

- macOS platform-node code is unverified on real hardware (ratified, expected).
- `key-store.configure()` resets its env snapshot on every call — dormant risk for Phase 3.
- `anthropic.ts`/`openai.ts` have no dedicated test file — matches legacy's own boundary.
- Task 10's CLI copy (help text) isn't routed through the i18n catalog — file-boundary trade-off.
- Masked key-input TTY path verified via mock-TTY simulation of real readline internals, not a
  genuine OS pty — one real-terminal smoke test recommended before treating it as fully closed.
- OWASP Low finding (unstripped control chars in AI free text) accepted as hardening debt.
- `opentimbre-packaging` domain skill still not created — propose when packaging work starts.
- Phases 2 (remaining plugins), 3 (Electron/Angular desktop), 4 (packaging/installers) not
  started — each needs its own plan, written after the previous phase validates.
