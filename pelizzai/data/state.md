...
- Task 10 ✅ 2026-08-08 — complete settings/plugin + real backend: handlers return camelCase Result<AppState> (legacy envelope deleted), safeStorage Vault adapter + core key-store wired (keys:save/remove encrypt/decrypt, KeyInfo only), real openai+anthropic providers wired so chat works from a saved key, persisted guitar, config:guitar validation, AppState.pluginIds; renderer GuitarForm + AiSettings (forced-provider disable, key inputs clear on success/retain on fail, unencrypted warning) + PluginBar (open/mapping actions); desktop README; sender validation in every handler; 45 main + 44 renderer + 10 e2e tests, check+lint+build+e2e+audit clean, split review (2 rounds) + OWASP approved
