...
O campo `note` é uma frase curta e útil (abordagem, captador recomendado,
...
- **`title`** — nome curto do trecho, 1 a 3 palavras, como rótulo de banco de
  patch: `Base de riffs`, `Solo`, `Intro limpa`. Sem artista, sem música.
- **`summary`** — **uma** linha de até ~60 caracteres dizendo o que a cena faz em
...
- **`explanation`** — 2 a 4 frases sobre **por que** este amp, este nível de
  ...
  reescrever o `summary` com outras palavras.
- **`guitar`** — o que fazer no instrumento: captador, volume, tone e uma
...
- Na `explanation` da cena que mudou, diga o que foi alterado e por quê.
...
detalhe já está na `explanation` de cada cena.
